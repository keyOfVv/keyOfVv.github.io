...
* 单个csv文件只有30M左右, 大大缩短读取的时间;
* 利用多核心CPU的多线程技术完成多csv文件的同步读取;
* 通过随机打乱文件的读取顺序, 实现每批素材的随机排列(第二部分将提及), 有助于提高训练效果;
...
对于CNN卷积网络模型的训练来说, 素材库优化是一个非常重要的环节, 合理的结构能提高训练效率, 降低训练总时长, 加快模型迭代. 在使用GPU执行训练时, 这一点尤为明显. 通常情况下, GPU运算速度远高于CPU加载素材的速度, 提高素材加载效率, 不但能有效缓解GPU的饥饿状态, 更能直接缩短训练周期. 
